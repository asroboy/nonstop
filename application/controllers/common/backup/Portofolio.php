...
class Portofolio extends CI_Controller{
	...
	function __construct(){
		...
		if($this->session->userdata('status') != $this->conf_status->stat() || $this->session->userdata('uri_portofolio') == 'N'){
	...
	public function Add(){
		...
		$data['jenis_file'] = 'portofolio';
		$data['des'] = $this->Model_action->trim($this->input->post('des'));
		if (!empty($_FILES['file']['name'])) {
			$file= $this->Model_action->upload('img/portofolio/', 500, 'file');
			$data['file'] = 'img/portofolio/'.trim($file);
		}
