        ...
        <ol class="breadcrumb float-sm-right">
          ...
          <?php
            if (empty($this->uri->segment(5))) {
              ...
              $service = $this->Service_M->getData($this->uri->segment(5));
              $s_service = $service->row_array();
              echo '<li class="breadcrumb-item"><a href="'.base_url("Panel/URI/".$this->uri->segment(3)).'">'.$menu.'</a></li>';
              echo '<li class="breadcrumb-item active">'.$s_service['judul'].'</li>';
            }
          ?>
        </ol>
      </div>
...
<?php
  if (empty($this->uri->segment(5))) {
    include 'layanankami.add.php'; 
  }else{
    if ($service->num_rows() > 0) {
      include 'layanankami.edit.php'; 
    }else{
