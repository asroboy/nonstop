    ...
    <div class="col-12">
      ...
      <div class="card" style="display: none" id="form-add">
        <div class="card-body pad">
          <form action="<?php echo base_url('common/SubMenu/Add')?>" method="POST" enctype="multipart/form-data" id="frm_crud">
            <div class="form-group row">
              <div class="col-md-4" style="margin-bottom: 10px;">
                ...
                <select name="id_menu" class="form-control" required="">
                  ...
                  <?php
                    $menu = $this->MainMenu_M->All()->result_array();
                    foreach ($menu as $s_menu) {
                      echo '<option value="'.$s_menu['id'].'">'.$s_menu['nama'].'</option>';
                    }
                  ?>
                </select>
              </div>
    ...
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-body table-responsive">
          <?php 
            $data = array('jumnya' => $this->SubMenu_M->All()->num_rows());
          ...
          <table class="table table-hover table-bordered table-striped example1">
            ...
            <tbody>
              <?php
                ...
                $menu = $this->SubMenu_M->Show()->result_array();
                foreach ($menu as $s_menu) {
              ?>
              <tr>
                <td align="center"><?php echo $no++?></td>
                <td>
                  <?php echo $s_menu['nama']?></br>
                  <small><label><b>
                    Main Menu : <?php echo $this->MainMenu_M->getDetail($s_menu['id_menu'], 'id', 'nama')?> / <?php echo $s_menu['urutan']?></br>
                    Type : <?php echo $s_menu['type']?></br>
                    Target : <?php echo $s_menu['target']?>
                  </b></label></small>
                </td>
                <td>
                  <?php 
                    if ($s_menu['type'] == 'static page') {
                      echo $this->Halaman_M->getDetail($s_menu['url'],'id','url');
                    }else{
                      echo $s_menu['url'];
                    }
                  ?>
                </td>
                <td><?php echo $this->Model_main->made($s_menu['create_by'])?></br><small><?php echo $this->Model_main->tgl($s_menu['tgl']);?></small></td>
                <td>
                  <a href="<?php echo base_url('Panel/URI/SubMenu/edit/'.$s_menu['id'])?>" class="btn btn-warning btn-sm">Edit</a>
                  &nbsp;&nbsp;&nbsp;
                  <a href="<?php echo base_url('common/SubMenu/Delete/'.$s_menu['id'])?>" class="btn btn-danger btn-sm" onclick="return confirm('Apa anda ingin menghapus data ( <?php echo $s_menu['nama']?> ) ?')">Delete</a>
                </td>
              </tr>
              <?php } ?>
            </tbody>
