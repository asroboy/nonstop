        ...
        <ol class="breadcrumb float-sm-right">
          ...
          <?php
            if (empty($this->uri->segment(5))) {
              ...
              $sb = $this->SliderBanner_M->getData($this->uri->segment(5));
              $s_sb = $sb->row_array();
              echo '<li class="breadcrumb-item"><a href="'.base_url("Panel/URI/".$this->uri->segment(3)).'">'.$menu.'</a></li>';
              echo '<li class="breadcrumb-item active">'.$s_sb['judul'].'</li>';
            }
          ?>
        </ol>
      </div>
...
<?php
  if (empty($this->uri->segment(5))) {
    include 'sliderbanner.add.php'; 
  }else{
    if ($sb->num_rows() > 0) {
      include 'sliderbanner.edit.php'; 
    }else{
