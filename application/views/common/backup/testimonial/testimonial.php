        ...
        <ol class="breadcrumb float-sm-right">
          ...
          <?php
            if (empty($this->uri->segment(5))) {
              ...
              $testimoial = $this->Testimonial_M->getData($this->uri->segment(5));
              $s_testimoial = $testimoial->row_array();
              echo '<li class="breadcrumb-item"><a href="'.base_url("Panel/URI/".$this->uri->segment(3)).'">'.$menu.'</a></li>';
              echo '<li class="breadcrumb-item active">'.$s_testimoial['nama'].'</li>';
            }
          ?>
        </ol>
      </div>
...
<?php
  if (empty($this->uri->segment(5))) {
    include 'testimonial.add.php'; 
  }else{
    if ($testimoial->num_rows() > 0) {
      include 'testimonial.edit.php'; 
    }else{
