        ...
        <ol class="breadcrumb float-sm-right">
          ...
          <?php
            if (empty($this->uri->segment(5))) {
              ...
              $query = $this->Bantuan_M->getData($this->uri->segment(5), 'id', 'bantuan');
              $show = $query->row_array();
              $userApp = $this->UserApp_M->getData($show['user_id'])->row_array();
              echo '<li class="breadcrumb-item"><a href="'.base_url("Panel/URI/".$this->uri->segment(3)).'">'.$menu.'</a></li>';
              echo '<li class="breadcrumb-item active">'.$show['title'].'</li>';
            }
          ?>
        </ol>
      </div>
...
<?php
  if (empty($this->uri->segment(5))) {
    include 'bantuan.add.php'; 
  }else{
    if ($query->num_rows() > 0) {
      include 'bantuan.jawab.php'; 
    }else{
