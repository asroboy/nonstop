        ...
        <ol class="breadcrumb float-sm-right">
          ...
          <?php
            if (!empty($this->input->get('query'))) {
              $link = base_url('Panel/URI/Diskusi?query='.$this->input->get('query'));
            }else{
              $link = base_url('Panel/URI/Diskusi');
            }

            if (empty($this->input->get('id'))) {
              echo '<li class="breadcrumb-item active">'.$menu.'</li>';
            }else{
              $query = $this->Diskusi_M->getData($this->input->get('id'));
...
<?php
  if (empty($this->input->get('id'))) {
    include 'diskusi.add.php'; 
  }else{
    if ($query->num_rows() > 0) {
      include 'diskusi.detail.php'; 
    }else{
